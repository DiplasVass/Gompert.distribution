...
Dat15 = replicate(L,rgompertz(15, scale = 1, shape=3))
...
Dat50 = replicate(L,rgompertz(50, scale = 1, shape=3))
...
Dat100 = replicate(L,rgompertz(100, scale = 1, shape=3))
...
TABLE4 = rbind(tb15,tb50,tb100)
colnames(TABLE4) = c('MLE','UMVUE','MSE','JEFFREYS'
                   ,'GAMMA(gama=0.8,delta=0.5)','GAMMA(gama=0.8,delta=3.0)'
                   ,'GAMMA(gama=3.0,delta=0.5)','GAMMA(gama=3.0,delta=3.0)')
rownames(TABLE4) = c('n=15','n=50','n=100')
TABLE4
...
(1/36) + (1/18) + (1/9) + (1/12) + (5/36)

v= c(1/36,1/18,1/12,1/9,5/36)
sum(v)
