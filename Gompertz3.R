...
dat15 = rgompertz(n=15, scale = 1, shape=3.0)

## Monte Carlo Simulations (L = 5000) for n=15

Dat15 = replicate(L,rgompertz(15, scale = 1, shape=3.0))
...
r15 = cbind(mean(phimle15),mean(phiumv15),mean(phimse15),3.0)
r15
...
dat50 = rgompertz(n=50, scale = 1, shape=3.0)

## Monte Carlo Simulations (L = 5000) for n=50

Dat50 = replicate(L,rgompertz(50, scale = 1, shape=3.0))
...
r50 = cbind(mean(phimle50),mean(phiumv50),mean(phimse50),3.0)
r50
...
dat100 = rgompertz(n=100, scale = 1, shape=3.0)

## Monte Carlo Simulations (L = 5000) for n=50

Dat100 = replicate(L,rgompertz(100, scale = 1, shape=3.0))
...
r100 = cbind(mean(phimle100),mean(phiumv100),mean(phimse100),3.0)
r100
...
b15 = cbind(mean(phibg15),mean(phibj15),3.0)
b15
...
b50 = cbind(mean(phibg50),mean(phibj50),3)
b50
...
b100 = cbind(mean(phibg100),mean(phibj100),3)
b100
...
sample = c(15,15,50,50,100,100)

TABLE2 = matrix(c(sample,co1.1,co1.2,co1.3,co2,co3,co4,co5,co6),6)
rownames(TABLE2) = c('n=15  EXP','n=15  MSE','n=50  EXP','n=50  MSE','n=100 EXP','n=100 MSE') 
colnames(TABLE2) = c('Criteria     n','MLE','UMVUE','MSE','Jeffreys prior'
...
# First curve is plotted
 plot(xdata, y1, type="o", col="blue", pch="o", lty=1,ylim=c(2.5,3.5) 
      ,xlab='n',ylab='Empirical  Values of Statistical Estimators')
...
# Use symbol '+' for points.
 ...
 legend(60,3.5,legend=c('MLE','UMVUE','MSE'),col=c('blue','red','dark red'),lty=c(1,2,3),box.lty=0)
...
# plot the first curve by calling plot() function
# First curve is plotted
 plot(xdata, u1, type="o", col="blue", pch="o", lty=1,ylim=c(1.5,3.5) 
      ,xlab='n',ylab='Empirical  Values of Bayes Estimators')

# Add second curve to the same plot by calling points() and lines()
# Use symbol '*' for points.
 points(xdata, u2, col="red", pch="*")
 lines(xdata, u2, col="red",lty=2)
 abline(v=ss,lty=2)
# Add Third curve to the same plot by calling points() and lines()
# Use symbol '+' for points.
 points(xdata, y3, col="dark red",pch="+")
...
# Use symbol '^' for points.
 points(xdata, u4, col="purple",pch="^")
 lines(xdata, u4, col="purple", lty=3)
# Add Fourth curve to the same plot by calling points() and lines()
# Use symbol 'r' for points.
 points(xdata, u5, col="green",pch="r")
 lines(xdata, u5, col="green", lty=3) 
 legend(60,3.5,legend=c('Jeffreys','Gamma(0.8,0.5)','Gamma(0.8,3.0)'
         ,'Gamma(3.0,0.5)','Gamma(3.0,3.0)')
        ,col=c('blue','red','dark red','purple','green'),lty=c(1,2,3),box.lty=0)
